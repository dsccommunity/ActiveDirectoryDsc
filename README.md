...
The xADOptionalFeature DSC resource will enable the Active Directory Optional Features such as Recycle Bin feature for the target forest.
This resource first verifies that the forest mode matches the features requirement or is greater.  If the forest mode
...
* **`[String]` ForestFQDN** _(Key)_:  Fully qualified domain name of forest to enable Active Directory Optional Feature.
* **`[String]` FeatureName** _(Key)_:  Name of the feature to be enabled.
...
* **`[Boolean]` Enabled** _(Read)_:  Read-only. Returned by Get.
...
* **`[String]` FeatureName** _(Read)_:  Name of the feature to be enabled.
* **`[PSCredential]` EnterpriseAdministratorCredential** _(Required)_:  Credential with Enterprise Administrator rights to the forest.
* **`[Boolean]` Enabled** _(Read)_:  Read-only. Returned by Get.
